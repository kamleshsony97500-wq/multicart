...
const SupportChats = () => {
    ...
    useEffect(() => {
        const fetchChatMessages = async () => {
            try {
                const result = await axios.post("/api/support/get",
    ...
    return (
        <div className='min-h-screen bg-linear-to-br from-gray-900 via-black to-gray-900 text-white p-3 sm:p-6'>
            ...
            {/* left side area div */}
            <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 h-[90vh]">
                <div className="bg-black/50 border border-white/20 p-4 rounded-2xl overflow-y-auto">
                    <h2 className="mb-4 text-white font-semibold text-lg">
                        Support Chats
                    </h2>
                    {userData?.role !== "admin" && <p className="text-xs my-2 rounded bg-yellow-800 p-3 text-yellow-300 mt-1 leading-relaxed">
                        {userData?.role === "user" && (
                            <>
                                Note: The vendor's response may take 1-2 hours.
                        ...
                        {userData?.role === "vendor" && (
                            <>
                                Note: The admin's response may take 1-2 hours.
                                In some cases, you may receive a reply sooner.
                            </>
                        )}
                    </p>}
                ...
                <div className="md:col-span-2 bg-black/50 border border-white/10 rounded-2xl flex flex-col overflow-hidden">
                    {!activeUser ? (<div className="flex-1 flex items-center justify-center text-gray-400">
                        ...
                        <>
                                ...
                                {messages.map((msg, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {/* MESSAGE */}
                                            <div
                                                className={`max-w-[70%] px-4 py-2.5 text-sm rounded-2xl ${isMe
                            ...
                            {suggestions.length > 0 && <div className="px-4 pb-2 flex flex-wrap gap-2">
                                {
                                    suggestions.map((s, i) => (
                                        ...
                                        bg-blue-500/10 text-blue-300 border border-blue-500/30 hover:bg-blue-500/20 transition">
                                            {s}
                                        </div>
                                    ))
                                }
                            ...
                            <div className="p-3 border-t border-white/10 bg-black/60 flex gap-2">
                                <input
                                    ...
                                    className="flex-1 bg-black/80 text-white border border-white/20 rounded-full px-5 py-2.5 outline-none focus-within:border-blue-500"
                                />
