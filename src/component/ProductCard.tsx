...
const ProductCard = ({ product }: { product: IProduct }) => {
  ...
  return (
    <motion.div
      ...
      className='bg-white shadow-md rounded-xl overflow-hidden border hover:shadow-xl transition cursor-pointer'>
      {/* images */}
      <div className='relative w-full h-55 bg-gray-100 overflow-hidden flex items-center justify-center'>
        ...
        <button
          ...
          className='absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 rounded-full text-white z-10 p-2 cursor-pointer'>
        ...
        <button
          ...
          className='absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 rounded-full text-white z-10 p-2 cursor-pointer'>
          <FaChevronRight size={14} />
        </button>
        <div className='absolute bottom-2 left-1/2 -translate-y-1/2 flex gap-1'>
          {images.map((_, i) => (
            <span key={i} className={`w-2 h-2 rounded-full ${current === i ? "bg-black" : "bg-black/40"
              }`}>
      ...
      <div className='p-2 space-y-2'>
        ...
        <p className='text-xs text-gray-500'>Sold by: {' '}
          <span>{product.vendor.shopName}</span></p>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className='w-full mt-3 bg-black text-white py-2 rounded-lg gap-2 flex items-center justify-center hover:bg-gray-900 transition cursor-pointer'>
