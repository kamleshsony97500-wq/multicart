...
const VendorProduct = () => {
  ...
  const toggleIsActive = async (productId: string, currentIsActive: boolean) => {
    try {
      ...
      const updatedProducts = allProductsData.map((p: any) => p._id === productId ? result.data : p)
            ...
            {myProducts.length === 0 ? (
              ...
              myProducts.map((p, index) => (
                <tr key={index} className="border-t border-white/10 hover:bg-white/5">
                  <td className="p-4">
                    <Image src={p?.image1} alt='image' width={50} height={50}
                      className='rounded object-cover' />
                  </td>
                  <td className="p-4">{p?.title.slice(0, 90) + "..."}</td>
                  <td className="p-4">₹ {p?.price}</td>
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-full text-xl bg-gray-500/30 ${{
                      approved: "text-green-400",
                      pending: "text-yellow-400",
                      rejected: "text-red-400",
                    }[p?.verificationStatus] || "text-gray-400"
                      }`}
                    >
                      {p?.verificationStatus}</span></td>

                  <td className="p-4">
                    <span className={`text-sm ${p.isActive
                      ? "text-green-400"
                      : "text-red-400"
                      }`}>
                  ...
                  <td className="p-4 text-center">
                    <div className='flex space-x-2 items-center justify-center'>
                      <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                        className='px-3 py-1 cursor-pointer rounded text-sm bg-purple-600 hover:bg-purple-700'
                        onClick={() => router.push(`/updateProduct/${p._id}`)}>                    Edit
                      </motion.button>
                      ...
                      <motion.button
                        ...
                        className={`px-3 py-1 rounded text-sm  ${p?.verificationStatus === "approved"
                    ...
                    {p?.verificationStatus === "rejected" &&
                      <div className='mt-2 bg-red-500/10 border border-red-500/30 text-gray-300 text-xs p-2 rounded'>
                        <p>
                          <b>Rejected:</b> {" "}
                          {p?.rejectedReason || "No reason provided"}
                        </p>
                        <p className='mt-1 text-yellow-300'>
                          ✏️After edit, product will be sent for
                          re-verification.
                        </p>
                      </div>}
          ...
          myProducts.map((p, index) => (
            <div key={index} className="bg-white/10 border border-white/20 rounded-xl p-4 space-y-2">
              <div className='flex items-center gap-3'>
                <Image src={p?.image1} alt='product' width={60} height={60}
                  className='rounded' />
              </div>

              <div>
                <h2 className='font-semibold'>{p?.title.slice(0, 100) + "..."}</h2>
